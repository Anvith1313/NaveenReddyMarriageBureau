...
function ProfileCard({ p, saved, myInterest, onSave, onInterest, onWhatsapp }: {
  ...
  return (
    <div className={s.pcard}>
      <div className={s.pcardHead} style={{ background: (p.bg as string) ?? '#f5e6e8' }}>
      ...
      <div className={s.pcardBody}>
        ...
        <div className={s.detGrid}>
          ...
          {contactOk && (
            <div className={s.diVFull}><div className={s.diL}>📞 Mobile</div><div className={s.diV}>{p.mob}</div></div>
          )}
          {contactOk && (
            <div className={s.diVFull}><div className={s.diL}>✉ Email</div><div className={s.diV}>{(p as Record<string, unknown>).email as string}</div></div>
          )}
        </div>
        {!contactOk && (
          <div className={s.privNote}>Contact details are strictly private and will only be disclosed after mutual interest and bureau administrator approval.</div>
        )}
        <div className={s.pactRow}>
          {!isMutual && (isPending ? (
